...
export class Sheep {
    ...
    moving: boolean;
    isLamb: boolean;
    minDistanceFromHerd: number;
    outerReactionLimit: number;
     ...
     * @param {Number} yPos
     */
    constructor(xPos: number, yPos: number, id: number, isLamb: boolean) {
        this.xPos = xPos;
        this.yPos = yPos;
    ...
    update(
        ...
        // Calculate the sheep's velocity towards the center of the herd
        let xHerdVel: number, yHerdVel: number;
        [xHerdVel, yHerdVel] = this.getVelocityTowardHerd(herdXCenter, herdYCenter);
        
        // Calculate the sheep's velocity away from the dog.
        let xDogVel: number, yDogVel: number;
        [xDogVel, yDogVel] = this.getVelocityAwayFromDog(dog);

        // Check if the sheep is too close to any side, and if so, give it a 
        // velocity in the opposite direction
        let xSideVel: number, ySideVel: number;
        [xSideVel, ySideVel] = this.getVelocityAwayFromSides();

        // Sum the velocities and update the sheep's position
        ...
        [combinedXVel, combinedYVel] = this.checkMoveForObstacles(
            combinedXVel, combinedYVel, obstacles
        );
    ...
    /**
     * Calculates the sheep's velocity toward the center of the herd,
     ...
     */
    getVelocityTowardHerd(herdXCenter: number, herdYCenter: number) {
        let distToHerd = getDistanceToPoint(this.xPos, this.yPos, herdXCenter, herdYCenter);
        if (distToHerd > this.minDistanceFromHerd) {
            let directionToHerdCenter = getDirectionToPoint(this.xPos, this.yPos, herdXCenter, herdYCenter);
     ...
     * @returns an array containing the x and y velocity components.
     */
    getVelocityAwayFromDog(dog: Dog) {
     ...
     */
    checkMoveForObstacles(xVel: number, yVel: number, obstacles: Obstacle[]) {
        let futureX = this.xPos + xVel;
        let futureY = this.yPos + yVel;
        for (let obstacle of obstacles) {
            let rect = new Rectangle(obstacle.x, obstacle.y, obstacle.x + obstacle.width, obstacle.y + obstacle.height);
